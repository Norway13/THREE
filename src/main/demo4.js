...
let axesHelper, spotlightHelper, gui;
...
function buildGUI() {
    ...
    spotLightFolder.addColor(spotLight, 'color').onChange((val) => {
        spotLight.color.set(val)
        render()
    })
    spotLightFolder.add(spotLight, 'angle', 0, Math.PI / 2).onChange((val) => {
        spotLight.angle = val
        render()
        spotlightHelper.update()
    })
    spotLightFolder.add(spotLight, 'penumbra', 0, 1).onChange((val) => {
    ...
    cameraFolder.add(camera.position, 'x', -1000, 1000).step(1).onChange((val) => {
